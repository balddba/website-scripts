/*******************************************************************************
*
* Script Name: dg_standby.sql
* Title: Data Guard standby snapshot
* Tags: Data Guard, Standby, Diagnostics
* Purpose: Print standby-side Data Guard identity, redo, files, and apply status for comparison with the primary
*
* Description:
*   Run this on each physical standby and dg_primary.sql on the primary, spool
*   both outputs, and compare. The comparison fingerprint, datafile inventory
...
*   V$/GV$ views so it works on a mounted standby as well as Active Data Guard.
*   On a CDB, run it in the root.
*
* Parameters:
*   None.
*
...
*   - SELECT on GV$MANAGED_STANDBY, GV$DATAGUARD_PROCESS, V$ARCHIVE_GAP
*   - SELECT on V$RECOVERY_PROGRESS, GV$PDBS, V$FLASHBACK_DATABASE_LOG
*
* Output Format:
*   - Role check and comparison fingerprint (name/value)
*   - Instances, protection, incarnation, NLS, parameters
*   - Archive destinations and apply status
*   - Online and standby redo logs, datafiles, tempfiles, PDBs
*   - Apply/transport lag, archive gaps, and recent Data Guard messages
*
* Example Usage:
*   SQL> SPOOL dg_standby.txt
*   SQL> @dg_standby.sql
*   SQL> SPOOL OFF
*
...
COLUMN open_mode_check FORMAT A70
...
COLUMN last_archived_seq FORMAT 999999990 HEADING 'Last Recv'
COLUMN last_applied_seq FORMAT 999999990 HEADING 'Last Appl'
COLUMN last_next_time FORMAT A19 HEADING 'Last Time'
COLUMN received_seq FORMAT 999999990 HEADING 'Received'
COLUMN applied_seq FORMAT 999999990 HEADING 'Applied'
COLUMN apply_gap FORMAT 999999990 HEADING 'Gap'
COLUMN low_sequence# FORMAT 999999990 HEADING 'Low Seq'
COLUMN high_sequence# FORMAT 999999990 HEADING 'High Seq'
COLUMN stats_name FORMAT A32 HEADING 'Name'
COLUMN stats_value FORMAT A32 HEADING 'Value'
COLUMN unit FORMAT A16
COLUMN time_computed FORMAT A20
COLUMN datum_time FORMAT A20
COLUMN item FORMAT A32
COLUMN units FORMAT A16
COLUMN sofar FORMAT 999999999990
COLUMN total FORMAT 999999999990
...
COLUMN mrp_check FORMAT A70
COLUMN srl_check FORMAT A70

PROMPT
PROMPT ===============================================================================
PROMPT Data Guard standby snapshot
PROMPT ===============================================================================
...
SELECT
    CASE
        WHEN database_role = 'PHYSICAL STANDBY' THEN 'OK'
        WHEN database_role = 'LOGICAL STANDBY' THEN 'OK (logical standby; apply views below are for physical)'
        WHEN database_role = 'SNAPSHOT STANDBY' THEN 'WARNING: snapshot standby; redo apply is stopped'
        ELSE 'WARNING: expected PHYSICAL STANDBY, found ' || database_role
    END AS role_check,
    CASE
        WHEN open_mode IN ('MOUNTED', 'READ ONLY', 'READ ONLY WITH APPLY') THEN 'OK'
        ELSE 'WARNING: unexpected open_mode ' || open_mode
...
PROMPT ======================
PROMPT Compare this block with dg_primary.sql. DBID, name, resetlogs, compatible,
PROMPT block size, charset, file counts, file# checksum, and redo sizes must match.
PROMPT CURRENT_SCN on a physical standby is the last applied SCN.
...
PROMPT =====================
PROMPT LAST RECV is the newest archived sequence on this standby. LAST APPL
PROMPT is the newest sequence MRP has applied. Compare LAST APPL with the
PROMPT primary's last archived sequence.
...
PROMPT
PROMPT Applied vs received by destination
PROMPT ==================================

SELECT
    dest_id,
    thread#,
    MAX(sequence#) AS received_seq,
    MAX(CASE WHEN applied = 'YES' THEN sequence# END) AS applied_seq,
    MAX(sequence#) - MAX(CASE WHEN applied = 'YES' THEN sequence# END) AS apply_gap
FROM v$archived_log
WHERE resetlogs_change# = (SELECT resetlogs_change# FROM v$database)
...
PROMPT =================
PROMPT Standby redo logs should exist, one more group per thread than the
PROMPT primary online logs, and at least as large as the largest online log.
...
PROMPT ==================
PROMPT Compare FILE#, GB, and STATUS with the primary. Names often differ
...
PROMPT ==================
PROMPT Tempfile count and paths can differ until standby tempfiles are created.
...
PROMPT
PROMPT Managed standby / apply processes
PROMPT =================================
...
PROMPT
PROMPT Apply and transport lag
PROMPT =======================

SELECT
    name AS stats_name,
    value AS stats_value,
    unit,
    time_computed,
    datum_time
...
PROMPT
PROMPT MRP and standby redo check
PROMPT ==========================

SELECT
    CASE
        WHEN (SELECT COUNT(*) FROM gv$managed_standby WHERE process LIKE 'MRP%') > 0
            THEN 'OK: MRP is running'
        WHEN (SELECT COUNT(*) FROM gv$dataguard_process WHERE name LIKE 'MRP%' OR role LIKE '%APPLY%') > 0
            THEN 'OK: apply process is running'
        ELSE 'WARNING: no MRP apply process found'
    END AS mrp_check,
    CASE
        WHEN (SELECT COUNT(*) FROM v$standby_log) = 0
            THEN 'WARNING: no standby redo logs; real-time apply cannot use SRLs'
...
PROMPT Archive gaps
PROMPT ============
PROMPT No rows means no gap is recorded in V$ARCHIVE_GAP.
...
PROMPT
PROMPT End of Data Guard standby snapshot
PROMPT
