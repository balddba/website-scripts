"""Integration test for oracle/sql/dg_gap_details.sql."""
...
@pytest.mark.oracle
def test_dg_gap_details(run_oracle_sql) -> None:
    """Execute dg_gap_details.sql and verify archive gap columns.

    Args:
        run_oracle_sql: Fixture to execute a SQL script.
    """
    try:
        result = run_oracle_sql("dg_gap_details.sql")
    except oracledb.DatabaseError as exc:
        if _is_environment_limitation(exc):
            pytest.skip(str(exc).split("\n", maxsplit=1)[0])
        raise

    query = _query_with_columns(result, EXPECTED_GAP_COLUMNS)
    assert query is not None, f"dg_gap_details.sql did not return query with columns {sorted(EXPECTED_GAP_COLUMNS)}"
