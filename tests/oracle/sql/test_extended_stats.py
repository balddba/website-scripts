"""Integration test for oracle/sql/extended_stats.sql."""
...
EXTSTAT_TABLE = "WS_SQLTEST_EXTSTAT_TAB"


@pytest.mark.oracle
def test_extended_stats_reports_column_group(run_oracle_sql, oracle_settings: OracleTestSettings) -> None:
    """Create a column group extension, gather stats, and verify extended_stats.sql output."""
    with seeded_extended_stats_table(oracle_settings) as schema:
        try:
            result = run_oracle_sql("extended_stats.sql", args=[schema])
        except oracledb.DatabaseError as exc:
            if _is_environment_limitation(exc):
                pytest.skip(str(exc).split("\n", maxsplit=1)[0])
            raise

    assert _has_value(result, "TABLE_NAME", EXTSTAT_TABLE)
    assert _has_row(
        result,
        {"OWNER": schema, "TABLE_NAME": EXTSTAT_TABLE, "DROPPABLE": "YES"},
    )


@contextmanager
def seeded_extended_stats_table(oracle_settings: OracleTestSettings) -> Iterator[str]:
    """Create a table with column group extended stats.
    ...
    try:
        with oracle_connection(oracle_settings) as connection, connection.cursor() as cursor:
            ...
            cursor.executemany(
                f"INSERT INTO {target} (col1, col2) VALUES (:1, :2)",
                [("A", "1"), ("B", "2"), ("C", "3")],
            )
            connection.commit()
            try:
                cursor.callfunc(
                    ...
                    [schema, EXTSTAT_TABLE, "(col1, col2)"],
                )
            except oracledb.DatabaseError as exc:
                if _is_environment_limitation(exc):
                    pytest.skip(str(exc).split("\n", maxsplit=1)[0])
                raise
            cursor.callproc("DBMS_STATS.GATHER_TABLE_STATS", [schema, EXTSTAT_TABLE])
        yield schema
    finally:
        with oracle_connection(oracle_settings) as connection, connection.cursor() as cursor:
            _drop_table(cursor, target)
