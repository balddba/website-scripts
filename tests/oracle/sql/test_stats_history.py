"""Integration test for oracle/sql/stats_history.sql."""
...
STATSHIST_TABLE = "WS_SQLTEST_STATSHIST_TAB"


@pytest.mark.oracle
def test_stats_history_reports_history_entries(run_oracle_sql, oracle_settings: OracleTestSettings) -> None:
    """Create a table, gather stats multiple times, and verify history output."""
    with seeded_stats_history_table(oracle_settings) as schema:
        try:
            result = run_oracle_sql("stats_history.sql", args=[STATSHIST_TABLE])
        except oracledb.DatabaseError as exc:
            if _is_environment_limitation(exc):
                pytest.skip(str(exc).split("\n", maxsplit=1)[0])
            raise

    assert _has_value(result, "TABLE_NAME", STATSHIST_TABLE)
    assert _has_row(
        result,
        {"OWNER": schema, "TABLE_NAME": STATSHIST_TABLE},
        required={"STATS_UPDATE_TIME"},
    )


@contextmanager
def seeded_stats_history_table(oracle_settings: OracleTestSettings) -> Iterator[str]:
    """Create and seed a table, then gather stats twice to generate history entries.
    ...
    target = f"{schema}.{STATSHIST_TABLE}"
    try:
        with oracle_connection(oracle_settings) as connection, connection.cursor() as cursor:
            ...
            cursor.executemany(
                ...
                [(i, f"hist-row-{i}") for i in range(1, 10)],
            )
            connection.commit()
            cursor.callproc("DBMS_STATS.GATHER_TABLE_STATS", [schema, STATSHIST_TABLE])
            cursor.executemany(
                f"INSERT INTO {target} (id, payload) VALUES (:1, :2)",
                [(i, f"hist-extra-{i}") for i in range(10, 20)],
            )
            connection.commit()
            cursor.callproc("DBMS_STATS.GATHER_TABLE_STATS", [schema, STATSHIST_TABLE])
        yield schema
